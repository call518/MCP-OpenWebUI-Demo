...
[[ -d src/mcp_openwebui_demo || -d src/$PKG_NAME ]] || die "src/mcp_openwebui_demo not found"
if [[ -d src/mcp_openwebui_demo && ! -d src/$PKG_NAME ]]; then mv src/mcp_openwebui_demo "src/$PKG_NAME"; fi
...
for f in $FILES; do
  ...
  grep -qE 'mcp_openwebui_demo|mcp-openwebui-demo|mcp-openwebui-demo' "$f" 2>/dev/null || continue
  sed -i -e "s/mcp_openwebui_demo/${PKG_NAME}/g" -e "s/mcp-openwebui-demo/${DIST_NAME}/g" -e "s/mcp-openwebui-demo/${DIST_NAME}/g" "$f" || true
done
